...
let vLive = 1, vBeta = 2;///, v22 = 19, v28 = 35, v31 = 42;
...
function showEnemies() {
  ...
  for (let s = 1; s <= versionEnemies.length; ++s) {
    ...
    sideHeader.innerHTML = `Side ${s} Lv70`;///(s != 4 ? `Trial ${s}` : `Adversity`) + ` Lv70`;
    ...
    // define enemy stats
    let eHP = Math.floor(sideHPMult * currEnemyData.baseHP[currEnemyType] * 24795 / 10000);///(s != 4 ? 8.74 : 15.8) * sideHPMult * currEnemyData.baseHP[currEnemyType] * 24795 / 10000);
    let eDEF = Math.ceil(currEnemyData.baseDEF[currEnemyType] * 1588 / 100);
    let eDaze = currEnemyData.baseDaze[currEnemyType] * 3.88;
...
function generateWR(mult, wr, id) {
  ...
  let wkCnt = resCnt = 0;
  for (let e = 0; e < elementsData.length; ++e) {/// - (versionNum < v28); ++e) {
  ...
  if (id == "24300") {///versionNum >= v28 && id == "24300") {
...
function generateEnemyStats(daze, stun, time, anom, dmg, mods) {
  ...
  if (mods.includes("no-anom")) return stats + `<span style="font-weight:bold;">IMMUNE TO ANOMALY</span>`;
    ...
    for (let i = 0; i < elementsData.length; ++i) stats += `<span style="color:${color[i]};font-weight:bold;">${Math.round(anom * anomMult[i] * (1 / (2 - Math.min(dmg[i], 1.2))) * 100) / 100}</span>/`;/// - (versionNum < v28); ++i) stats += `<span style="color:${color[i]};font-weight:bold;">${Math.round(anom * anomMult[i] * (1 / (2 - Math.min(dmg[i], 1.2))) * 100) / 100}</span>/`;
...
function loadSavedState() {
  ...
  versionNum = parseInt(localStorage.getItem("lastAZVersion") || `${vLive}`);
...
function saveProgress() {
  localStorage.setItem("lastAZVersion", versionNum);
